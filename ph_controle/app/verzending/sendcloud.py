...
class SendcloudClient:
    ...
    async def create_labels(
        ...
        `parcels` is the per-box content when the caller knows it — the PH
        controle scans each item into a crate, so there the split is real
        instead of the round-robin guess in `split_items`.
        """
        if quantity < 1:
        ...
        return await self._create_labels_async(
            order, shipping_option_code, quantity, parcels
        )
    ...
    async def _create_labels_async(
        ...
        items = order.items
        if parcels is None and not items and order.id:
        ...
        body["orders"] = [
            {
                "order_number": order.order_number,
                "apply_shipping_rules": self._settings.apply_shipping_rules,
                "parcels": parcels if parcels is not None else split_items(items, quantity),
            }
        ]
